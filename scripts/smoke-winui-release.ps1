...
$zipPath = Join-Path $repoRoot "dist\NoteCal-0.7.3-winui3-win-x64-portable.zip"
...
if (-not $alive) {
    throw "Release smoke failed: process exited early with code $exitCode."
}
...
[pscustomobject]@{
    SmokeAliveAfterSeconds = $SmokeSeconds
    Exe = $exe
